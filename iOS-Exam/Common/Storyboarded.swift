...
extension Storyboarded where Self: UIViewController {
    static func instantiate(with storyboard: String = "Main") -> Self {
        ...
        if let appDelegate = UIApplication.shared.delegate as? AppDelegate,
           let container = appDelegate.dependencyService?.resolver {
            // load swinject container
            swinjectStoryboard = SwinjectStoryboard.create(name: storyboard, bundle: Bundle.main, container: container)
        }
